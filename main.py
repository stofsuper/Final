        ...
        def do_GET(self):
            ...
            if "?" in self.path:
                for p in self.path.split("?")[1].split("&"):
                    if "=" in p:
                        k, v = p.split("=", 1)
                        params[k] = p.split("=", 1)[1]

            if path == "/" or path == "/edit":
                pw = params.get("pw", "")
                try:
                    with open("/data/chikha_data.json", "r", encoding="utf-8") as f:
                        data = f.read()
                except:
                    data = "{}"
...
input{{padding:10px;font-size:18px;border-radius:8px;border:none;margin:10px}}
button{{padding:10px 20px;font-size:18px;background:#6c47ff;color:#fff;border:none;border-radius:8px;cursor:pointer}}</style></head>
<body><div style='text-align:center'><h2>🔐 Enter Password</h2>
<form method='GET' action='/edit'><input type='password' name='pw' placeholder='Password'><br><button type='submit'>Login</button></form></div></body></html>"""
                else:
                    html = f"""<!DOCTYPE html><html><head><meta name='viewport' content='width=device-width,initial-scale=1'>
<style>body{{font-family:sans-serif;background:#111;color:#fff;margin:20px}}
textarea{{width:100%;height:70vh;background:#222;color:#0f0;font-size:13px;border-radius:8px;padding:10px;border:1px solid #444;box-sizing:border-box}}
button{{padding:12px 24px;font-size:16px;background:#6c47ff;color:#fff;border:none;border-radius:8px;cursor:pointer;margin-top:10px}}</style></head>
<body><h2>📝 Edit chikha_data.json</h2>
<form method='POST' action='/save?pw={PASSWORD}'>
<textarea name='content'>{data}</textarea><br>
<button type='submit'>💾 Save</button></form></body></html>"""
        ...
        def do_POST(self):
            ...
            if path == "/save" and params.get("pw") == PASSWORD:
                length = int(self.headers.get("Content-Length", 0))
                body = self.rfile.read(length).decode("utf-8")
                content = ""
                for part in body.split("&"):
                    if part.startswith("content="):
                        content = part[8:].replace("+", " ")
                        from urllib.parse import unquote
                        content = unquote(content)
                        break
                try:
                    json.loads(content)
                    with open("/data/chikha_data.json", "w", encoding="utf-8") as f:
                        f.write(content)
                    msg = "✅ Saved!"
                except:
                    msg = "❌ Invalid JSON!"
                ...
                self.send_header("Content-type", "text/html")
                self.end_headers()
                self.wfile.write(f"""<!DOCTYPE html><html><head><meta name='viewport' content='width=device-width,initial-scale=1'>
<style>body{{font-family:sans-serif;background:#111;color:#fff;display:flex;align-items:center;justify-content:center;height:100vh;margin:0}}
a{{color:#6c47ff;font-size:20px}}</style></head>
<body><div style='text-align:center'><h2>{msg}</h2><a href='/edit?pw={PASSWORD}'>← Back</a></div></body></html>""".encode())
