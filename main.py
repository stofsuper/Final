...
class MyBot(BaseBot):
    def __init__(self):
        ...
        self.compliments = [
            '<#ff69b4>Nta/Nti zwina bzaf! ✨',
            '<#ff4500>Style dyalk 3la 3la! 🔥',
            '<#f1c40f>Tbarkllah 3lik! 💫',
            '<#bf00ff>Outfit dyalk fire! 👑',
            '<#00cfff>Nta nadi f blasa! 🌟',
            '<#00e676>Energy dyalek top! ⚡',
            '<#ff8c00>Nta star! ⭐',
            '<#9b59b6>Chefti 3lik mezyan! 💎',
            '<#ff69b4>Rak classe khoya! 👔',
            '<#f1c40f>Ma sha Allah 3lik! 🌙',
            '<#00e676>Tbarklah, nta/nti f7al! 💯',
        ]
    ...
    async def periodic_announcements(self):
        ...
        tips = [
            ...
            "💬 Kteb !compliment @username bach tfarho/ha!",
    ...
    async def on_chat(self, user: User, message: str):
        try:
            ...
            if low in ('!leaderboard', '!top', '!lb'):
                ...
                if countdown:
                    await self.highrise.chat(
                        ...
                        f"💡 Earn pts: 💬chat|💃dance|🎁tip|⏱️stay!"
                    )
                else:
                    await self.highrise.chat("💡 Earn pts: 💬chat|💃dance|🎁tip|⏱️stay!")
                return
            ...
            # ── COPY OUTFIT (owner/mod only — free items only) ────────
            if low.startswith('!copy '):
                if not self.is_owner_or_mod(user):
                    await self.highrise.chat(f"❌ @{user.username} — !copy is for admins only! 🔒")
                    return
                parts = msg.split()
                target_name = parts[1].lstrip('@') if len(parts) > 1 else None
                if not target_name:
                    await self.highrise.chat("Usage: !copy @username")
                    return
                room_users = await self.safe_get_room_users()
                target_user = next((u for u, _ in room_users if u.username.lower() == target_name.lower()), None)
                if not target_user:
                    await self.highrise.chat(f"❌ @{target_name} not found in room!")
                    return
                try:
                    outfit_resp = await self.highrise.get_user_outfit(target_user.id)
                    outfit = outfit_resp.outfit if hasattr(outfit_resp, 'outfit') else None
                    if not outfit:
                        await self.highrise.chat(f"❌ Could not get @{target_name}'s outfit!")
                        return
                    # Filter: only keep free items (id starts with "free_")
                    free_items = [item for item in outfit if item.id.startswith("free_")]
                    if not free_items:
                        await self.highrise.chat(f"❌ @{target_name} has no free items to copy!")
                        return
                    await self.highrise.set_outfit(free_items)
                    await self.highrise.chat(f"👗 Bot copied {len(free_items)} free item(s) from @{target_name}! ✨")
                except Exception as e:
                    await self.highrise.chat(f"❌ Could not copy outfit: {e}")
                    print(f"[CopyOutfit] Error: {e}")
                return


            if low.startswith('!compliment'):
                parts = msg.split()
                target = parts[1].lstrip('@') if len(parts) > 1 else user.username
    ...
    async def on_disconnect(self) -> None:
        """Called when the WebSocket drops — force exit so Wyspbytes restarts."""
        self.is_connected = False
        print("[DISCONNECT] Bot disconnected! Force-exiting so Wyspbytes restarts...")
        await asyncio.sleep(2)
        sys.exit(1)
...
if __name__ == "__main__":
    ...
    if not token or not room_id:
        raise ValueError("BOT_TOKEN and ROOM_ID environment variables must be set!")
