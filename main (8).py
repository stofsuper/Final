...
class MyBot(BaseBot):
    ...
    async def on_chat(self, user: User, message: str):
        try:
            ...
            # ── !info — anyone can check their own info, owner can check others ──
            if low == "!info" or low.startswith("!info "):
                parts = msg.split()
                # Determine target
                if len(parts) >= 2 and self.is_owner(user):
                    target = parts[1]
                else:
                ...
                if target in self.vip_timed:
                    remaining = self.vip_timed[target] - time.time()
                    if remaining > 0:
                        days = int(remaining // 86400)
                        hrs2 = int((remaining % 86400) // 3600)
                        timed_str = f"{days}d {hrs2}h left"
                is_mod = "🛡️ Yes" if target in self.moderators else "No"
                greeting = self.custom_greetings.get(target, "None")[:40]

                lines = [
                    f"👤 {target}",
                    f"⭐ Points: {pts}",
                    f"💎 Perm VIP: {perm}",
                    f"⏰ Timed VIP: {timed_str}",
                    f"🛡️ Mod: {is_mod}",
                    f"💬 {greeting}"
                ]

                # !infow = whisper version
            ...
            # ── DATA MANAGEMENT COMMANDS (Owner only) ────────────────
            if self.is_owner(user):

                # !data — show all data categories
                if low == "!data":
                    vip_perm = len(self.vip_permanent)
                    vip_timed = len(self.vip_timed)
                    points_count = len(self.user_ratings)
                    greetings_count = len(self.custom_greetings)
                    mods_count = len(self.moderators)
                    await self.highrise.chat(
                        f"📊 DATA OVERVIEW:\n"
                        f"👑 Perm VIP: {vip_perm}\n"
                        f"⏰ Timed VIP: {vip_timed}\n"
                        f"⭐ Points: {points_count} users\n"
                        f"💬 Greetings: {greetings_count}\n"
                        f"🛡️ Mods: {mods_count}\n"
                        f"---\n"
                        f"!viplist | !timedvip | !pointslist | !greetlist"
                    )
                    return

                # !viplist — show all permanent VIPs
                if low == "!viplist":
                    if self.vip_permanent:
                        vips = " | ".join(self.vip_permanent)
                        await self.highrise.chat(f"💎 PERM VIP:\n{vips}")
                    else:
                        await self.highrise.chat("💎 No permanent VIPs.")
                    return

                # !timedvip — show all timed VIPs with expiry
                if low == "!timedvip":
                    if self.vip_timed:
                        now = time.time()
                        lines = []
                        for u, exp in self.vip_timed.items():
                            ...
                            if remaining > 0:
                                days = int(remaining // 86400)
                                hours = int((remaining % 86400) // 3600)
                                lines.append(f"⏰ {u}: {days}d {hours}h")
                            else:
                                lines.append(f"❌ {u}: expired")
                        await self.highrise.chat("⏰ TIMED VIP:\n" + "\n".join(lines))
                    else:
                        await self.highrise.chat("⏰ No timed VIPs.")
                    return

                # !pointslist — show top 15 by points
                if low == "!pointslist":
                    sorted_pts = sorted(self.user_ratings.items(), key=lambda x: x[1], reverse=True)[:15]
                    lines = [f"{i+1}. {u}: {p}pts" for i, (u, p) in enumerate(sorted_pts)]
                    await self.highrise.chat("⭐ TOP POINTS:\n" + "\n".join(lines))
                    return

                # !greetlist — show all custom greetings
                if low == "!greetlist":
                    if self.custom_greetings:
                        lines = [f"{u}: {g[:30]}" for u, g in list(self.custom_greetings.items())[:10]]
                        await self.highrise.chat("💬 GREETINGS:\n" + "\n".join(lines))
                    else:
                        await self.highrise.chat("💬 No custom greetings.")
                    return
                ...
                if low.startswith("!addvip "):
                    ...
                    if len(parts) >= 3:
                        ...
                        try:
                            ...
                            self.vip_timed[target] = expiry
                            self.save_data()
                            days = int(hours // 24)
                            hrs = int(hours % 24)
                            await self.highrise.chat(f"✅ @{target} got VIP for {days}d {hrs}h!")
                        except:
                            await self.highrise.chat("❌ Usage: !addvip username hours")
                    else:
                        await self.highrise.chat("❌ Usage: !addvip username hours")
                    return
                ...
                if low.startswith("!removevip "):
                    ...
                    if len(parts) >= 2:
                        ...
                        removed = False
                        if target in self.vip_timed:
                            del self.vip_timed[target]
                            removed = True
                        if target in self.vip_permanent:
                            self.vip_permanent.discard(target)
                            removed = True
                        if removed:
                            self.save_data()
                            await self.highrise.chat(f"✅ VIP removed from @{target}")
                        else:
                ...
                if low.startswith("!addpermvip "):
                    ...
                    if len(parts) >= 2:
                        target = parts[1]
                        self.vip_permanent.add(target)
                        self.save_data()
                        await self.highrise.chat(f"💎 @{target} got Permanent VIP!")
                    return
                ...
                if low.startswith("!addpoints "):
                    ...
                    if len(parts) >= 3:
                        ...
                        try:
                            ...
                            self.save_data()
                            total = self.user_ratings[target]
                            await self.highrise.chat(f"✅ +{amount} points → @{target} (total: {total})")
                        except:
                            await self.highrise.chat("❌ Usage: !addpoints username amount")
                    else:
                        await self.highrise.chat("❌ Usage: !addpoints username amount")
                    return
                ...
                if low.startswith("!removepoints "):
                    ...
                    if len(parts) >= 3:
                        ...
                        try:
                            amount = int(parts[2])
                            current = self.user_ratings.get(target, 0)
                            self.user_ratings[target] = max(0, current - amount)
                            self.save_data()
                            total = self.user_ratings[target]
                            await self.highrise.chat(f"✅ -{amount} points → @{target} (total: {total})")
                        except:
                            await self.highrise.chat("❌ Usage: !removepoints username amount")
                    else:
                        await self.highrise.chat("❌ Usage: !removepoints username amount")
                    return
                ...
                if low.startswith("!setpoints "):
                    ...
                    if len(parts) >= 3:
                        ...
                        try:
                            ...
                            self.save_data()
                            await self.highrise.chat(f"✅ @{target} points set to {amount}")
                        except:
                            await self.highrise.chat("❌ Usage: !setpoints username amount")
                    return

                # !addgreeting <username> <greeting text>
                if low.startswith("!addgreeting "):
                    parts = msg.split(None, 2)
                    if len(parts) >= 3:
                        target = parts[1]
                        greeting = parts[2]
                        self.custom_greetings[target] = greeting
                        self.save_data()
                        await self.highrise.chat(f"✅ Greeting set for @{target}!")
                    else:
                        await self.highrise.chat("❌ Usage: !addgreeting username text")
                    return
                ...
                if low.startswith("!removegreeting "):
                    ...
                    if len(parts) >= 2:
                        ...
                        if target in self.custom_greetings:
                            ...
                            self.save_data()
                            await self.highrise.chat(f"✅ Greeting removed for @{target}")
                        else:
                            await self.highrise.chat(f"❌ No greeting for @{target}")
                    return

                # !checkuser <username> — full profile (owner only)
                if low.startswith("!checkuser "):
                    parts = msg.split()
                    if len(parts) >= 2:
                        target = parts[1]
                        pts = self.user_ratings.get(target, 0)
